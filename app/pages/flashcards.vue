        ...
        <div class="container mx-auto px-6 py-6">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              type="button"
              class="px-4 py-2 text-sm font-medium text-deepTeal/70 hover:text-deepTeal border border-deepTeal/20 rounded-full hover:bg-deepTeal/5 transition-all"
              @click="handleShuffle"
            >
          ...
          <main>
            ...
            <div v-else-if="filteredTerms.length === 0" class="text-center py-20">
              <div class="max-w-md mx-auto">
                <div class="w-20 h-20 rounded-full bg-terracotta/10 flex items-center justify-center mx-auto mb-6">
                  <svg class="w-10 h-10 text-terracotta" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                </div>
                <h3 class="text-2xl font-serif font-bold text-deepTeal mb-2">
                  Aucun terme trouvé
                </h3>
            ...
            <div v-else class="space-y-8">
              <!-- Progress -->
              <div class="flex items-center justify-between">
                <div class="text-sm text-deepTeal/70">
                  Carte <span class="font-semibold text-deepTeal">{{ currentIndex + 1 }}</span> sur
                  <span class="font-semibold text-deepTeal">{{ filteredTerms.length }}</span>
                </div>
                <div class="flex items-center gap-2 text-xs text-deepTeal/60">
                  <kbd class="px-2 py-1 bg-white rounded border border-deepTeal/20">←</kbd>
                  <kbd class="px-2 py-1 bg-white rounded border border-deepTeal/20">→</kbd>
                  <span>pour naviguer</span>
                  <kbd class="px-2 py-1 bg-white rounded border border-deepTeal/20">Espace</kbd>
                  <span>pour retourner</span>
                </div>
              </div>

              <!-- Progress Bar -->
              <div class="w-full h-2 bg-deepTeal/10 rounded-full overflow-hidden">
                <div
              ...
              <div class="flex items-center justify-center gap-4 pt-8">
                <button
                  ...
                  class="px-6 py-3 rounded-full border-2 border-deepTeal/20 text-deepTeal font-medium hover:bg-deepTeal hover:text-warmCream transition-all disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-deepTeal"
                  @click="previousCard"
                >
                  <span class="flex items-center gap-2">
                ...
                <button
                  ...
                  class="px-6 py-3 rounded-full bg-deepTeal text-white font-medium hover:bg-deepTeal/90 transition-all shadow-lg disabled:opacity-30 disabled:cursor-not-allowed"
                  @click="nextCard"
                >
                  <span class="flex items-center gap-2">
                    Suivant
